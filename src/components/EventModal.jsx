                            ...
                            <div className="flex justify-between items-start mb-6 sm:mb-10">
                                <div>
                                    <div className="flex items-center gap-2 mb-2">
                                        ...
                                        <span className="text-[9px] sm:text-[10px] font-black uppercase tracking-[0.3em] text-slate-400 dark:text-slate-500">
                                            Detalles de la Agenda
                                        </span>
                                    </div>
                                    <h3 className="text-2xl sm:text-5xl font-black text-slate-900 dark:text-white capitalize tracking-tighter">
                                        {format(date, 'EEEE d', { locale: es })}
                                    </h3>
                                    <p className="text-slate-400 dark:text-slate-500 capitalize text-sm sm:text-lg font-bold tracking-tight mt-0.5 sm:mt-1">
                                        {format(date, 'MMMM yyyy', { locale: es })}
                                    </p>
                                </div>
                                <button
                                    onClick={onClose}
                                    className="p-2 sm:p-3 bg-slate-50 dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-xl sm:rounded-2xl transition-all text-slate-400 dark:text-slate-500 hover:text-slate-900 dark:hover:text-white border border-slate-100 dark:border-slate-700"
                                >
                                    <X className="w-5 h-5 sm:w-6 sm:h-6" />
                                </button>
                            </div>

                            <div className="space-y-3 sm:space-y-4 mb-4 max-h-[50vh] sm:max-h-[40vh] overflow-y-auto pr-1 sm:pr-2 custom-scrollbar">
                                {dayEvents.length === 0 ? (
                                    <div className="text-center py-12 sm:py-20 bg-slate-50/50 dark:bg-slate-950/20 rounded-2xl sm:rounded-3xl border border-dashed border-slate-200 dark:border-slate-800 flex flex-col items-center">
                                        <div className="p-4 sm:p-6 bg-white dark:bg-slate-900 rounded-full mb-4 sm:mb-6 shadow-sm">
                                            <CalendarIcon className="w-8 h-8 sm:w-10 sm:h-10 text-slate-200 dark:text-slate-700" />
                                        </div>
                                        <p className="text-base sm:text-xl font-bold text-slate-300 dark:text-slate-600 tracking-tight">Sin actividades programadas</p>
                                    </div>
                                ) : (
                                    dayEvents.map((event, idx) => (
                                        <motion.div
                                            ...
                                            className={cn(
                                                "p-3.5 sm:p-6 rounded-2xl sm:rounded-3xl border flex items-center gap-3 sm:gap-6 transition-all hover:border-slate-300 dark:hover:border-slate-600 bg-white dark:bg-slate-900 shadow-sm",
